...
const errorMessages = {
  ...
  familyProfile: {
    ...
    invalid_enum_value: 'O perfil familiar deve ser: SINGLE, COUPLE, FAMILY ou RETIRED.',
  },
};

const baseClientSchema = z.object({
  advisorId: z.string({ required_error: errorMessages.advisorId.required_error }).uuid(),
  name: z.string({ required_error: errorMessages.name.required_error })
    .min(4, { message: errorMessages.name.too_small })
    .max(256, { message: errorMessages.name.too_big }),
  email: z.string({ required_error: errorMessages.email.required_error })
  ...
  age: z.number({ required_error: errorMessages.age.required_error })
    .gt(16, { message: errorMessages.age.too_small }),
